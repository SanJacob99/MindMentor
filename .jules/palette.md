...
## 2024-06-23 - Interactive Input States & Web Compatibility
**Learning:** Native `TextInput` components lack built-in focus styling across React Native web. Adding dynamic classNames based on an `isFocused` state (with blue borders and matching icon colors) drastically improves form usability. Additionally, setting `outlineStyle: 'none'` (cast `as any`) on web prevents the native web-browser focus ring from conflicting with custom container borders. Furthermore, duplicate JSX properties inside text inputs (like `keyboardType`) can sneak in over time and cause TypeScript TS17001 compilation errors without an error log indicating the source.
**Action:** Always conditionally track input focus states in React Native cross-platform apps, apply container-level styling, implement web-specific `outlineStyle: 'none'`, and consistently check for duplicate JSX props during refactoring.
