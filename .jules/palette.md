...
## 2025-10-31 - Input Focus States and Web Outlines
**Learning:** React Native `TextInput` components do not automatically display a clear focus ring (unlike web inputs). Wrapping them in a `View` that lacks dynamic border colors based on focus makes it difficult for keyboard users to track their position, severely impacting accessibility in forms. Furthermore, on React Native Web, standard `TextInput` components display a default browser focus outline that clashes with custom `View` borders if not explicitly removed.
**Action:** Always implement a visible focus state by applying dynamic borders to the input's wrapper `View` (e.g., using `onFocus` and `onBlur` to set an `isFocused` state). To prevent visual conflicts on the web, always explicitly apply `style={Platform.OS === 'web' ? { outlineStyle: 'none' } as any : undefined}` to the `TextInput`.
