...
## 2026-03-03 - Focus States & Default Outlines on Web\n**Learning:** React Native `TextInput` components require manual focus state management for interactive styling (e.g., dynamic border colors). However, on React Native Web, the browser's default focus ring (outline) often clashes with custom borders. Applying `outline: 'none'` via standard styles or Tailwind classes is frequently overridden or causes TypeScript errors.\n**Action:** Always implement a manual `isFocused` state to drive border color changes on TextInputs. Additionally, suppress the web-specific default outline by injecting `style={Platform.OS === 'web' ? { outlineStyle: 'none' } as any : undefined}`, using the `any` cast to bypass `TS2769` errors without modifying the global type definitions.
