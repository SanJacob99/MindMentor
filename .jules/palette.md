...
## 2024-05-18 - TextInput Focus States and Web Outlines\n**Learning:** When adding custom visual focus states (like border colors) to React Native `TextInput` components to improve cross-platform keyboard accessibility, `style={{ outlineStyle: 'none' }}` must be applied and cast as `any` on Web to prevent default browser focus rings from clashing with the custom border. Care must be taken to not accidentally remove existing mobile UX props (`keyboardType`, `autoComplete`, `textContentType`) during this process, as they often appear together and can be mistaken for duplicates if not inspected closely.\n**Action:** Always verify that native mobile input props are preserved when applying visual focus state wrappers, and explicitly handle the web outline using `Platform.OS` checks.
