...
## 2026-04-20 - TextInput Focus States & JSX Attributes
**Learning:** Adding custom visual focus states to `TextInput` (like changing the border color of the wrapping View or its icon) drastically improves UX, but requires explicit focus state tracking via `onFocus` and `onBlur`. In addition, native properties (like `keyboardType` or `autoComplete`) must not be duplicated, as this causes `TS17001` compilation errors. Finally, explicitly applying `style={{ outlineStyle: 'none' }}` is required for React Native Web compatibility when implementing custom borders.
**Action:** When adding custom focus states to React Native inputs, maintain a single source of truth for semantic properties, remove any duplicates, and always apply `outlineStyle: 'none'` using a `Platform.OS === 'web'` check to prevent clash with default browser styling.
