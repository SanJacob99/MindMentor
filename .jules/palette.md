...
## 2024-05-03 - React Native Web Focus Styles
**Learning:** React Native `TextInput` components on the web inherit default browser focus rings (outlines) which often clash with custom application styling (e.g., changing border colors on focus). Using Tailwind classes like `outline-none` is frequently overridden by `react-native-web`'s inline styles. Additionally, solely changing the border color can cause layout shifts if the border width changes or can be hard to see against certain backgrounds. Duplicate JSX properties like `keyboardType` or `autoComplete` might exist if copying code without checking and cause TS compilation errors.
**Action:** When creating custom focus states for `TextInput` on web, explicitly apply `style={{ outlineStyle: 'none' } as any}` to disable the default ring. Use conditional `boxShadow` applied to the *container* View (e.g., `style={Platform.OS === 'web' && isFocused ? { boxShadow: '0 0 0 2px rgba(59, 130, 246, 0.5)' } as any : undefined}`) alongside the React Native border color change (`border-blue-500`) to create a clear, accessible focus indicator without layout shifting. Ensure you check for duplicate properties.
