...
## 2025-02-28 - React Native Web Focus Rings
**Learning:** Adding custom visual focus states (like border color changes) to `TextInput` components can conflict with the default browser focus ring when running React Native on the web. Standard Tailwind classes like `outline-none` are often overridden by inline styles generated by `react-native-web`.
**Action:** Always explicitly apply `style={Platform.OS === 'web' ? { outlineStyle: 'none' } as any : undefined}` to `TextInput` components when implementing custom focus styling to ensure a clean, native-like experience across both web and mobile platforms without conflicting outlines.
