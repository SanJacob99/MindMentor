...
## 2024-10-24 - Custom Focus Rings on React Native Web
**Learning:** When adding focus borders to text inputs that apply to both mobile and web, the default browser focus ring on web can clash or override custom Tailwind borders. `outline-none` via Tailwind classes doesn't always reliably stick due to how RNW injects inline styles.
**Action:** When implementing custom visual focus states on React Native inputs, explicitly apply `style={{ outlineStyle: 'none' } as any}` conditionally for web (`Platform.OS === 'web'`), and use conditional `boxShadow` instead of just border changes to ensure accessible visibility without layout shifting.
